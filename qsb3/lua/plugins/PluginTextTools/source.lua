...
function PluginTextTools:Note(_Text)
...
function PluginTextTools:StaticNote(_Text)
    _Text = self:ConvertPlaceholders(self:Localize(_Text));
    if Swift:IsGlobalEnvironment() then
...
function PluginTextTools:Message(_Text)
    _Text = self:ConvertPlaceholders(self:Localize(_Text));
    if Swift:IsGlobalEnvironment() then
...
function PluginTextTools:ClearNotes()
    if Swift:IsGlobalEnvironment() then
...
function PluginTextTools:Localize(_Text)
...
function PluginTextTools:ConvertPlaceholders(_Text)
    local s1, e1, s2, e2;
    while true do
...
function PluginTextTools:SplicePlaceholderText(_Text, _Start)
...
function PluginTextTools:ReplaceColorPlaceholders(_Text)
...
Swift:RegisterPlugins(PluginTextTools);
