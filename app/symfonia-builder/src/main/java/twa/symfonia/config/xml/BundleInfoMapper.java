...
/**
 * Ließt die Informationen eines Bundles oder eines Ordner mit Bundles darin aus.
 * @author mheller
 *
 */
...
{
    /**
     * Ließt die Info-Datei eines Bundles aus und erstellt daraus ein BundleModel-Objekt.
    ...
    /**
     * Versucht alle Bundles in einem Ordner einzulesen und gibt im Erfolgsfall eine Liste von Bundles zurück.
    ...
    /**
     * Wendet einen XPath auf das übergebene XML-Dokument an und gibt das Ergebnis zurück.
    ...
    /**
     * Gibt eine Liste von Bundle-Ordnern zurück. Der Name eines Bundles muss immer entweder mit "bundle" oder
     * mit "addon" beginnen und es muss ein Dateiordner sein.
     * @param path Pfad des Ordners
        ...
        for (int i=0; i<listOfFiles.length; i++) {
            if (listOfFiles[i].isDirectory()) {
                if (!listOfFiles[i].getName().endsWith("/.") && !listOfFiles[i].getName().endsWith("/..")) {
                    ...
                    // if (listOfFiles[i].getName().contains("bundle") || listOfFiles[i].getName().contains("addon")) {
